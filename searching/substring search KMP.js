...
function longestPrefix(pat) {
  // create a table of size equal to the length of `pat`
  // table[i] will store the prefix of the longest prefix of the substring pat[0..i]
  let table = new Array(pat.length);
  let maxPrefix = 0;
  // the longest prefix of the substring pat[0] has length
  table[0] = 0;
  ...
  for (let i = 1; i < pat.length; i++) {
    ...
    // case 2. The last character of the longest prefix matches the current character in `pat`
    if (pat.charAt(maxPrefix) === pat.charAt(i)) {
      ...
      // pat = abc----abc
      // consider `i` to be the last character `c` in `pat`
      // maxPrefix = will be 2 (the first `c` in `pat`)
...
function kmpMatching(str, pattern) {
  ...
  return console.log(matches.length);
}

kmpMatching("olegolegolegolegol", "ole");
